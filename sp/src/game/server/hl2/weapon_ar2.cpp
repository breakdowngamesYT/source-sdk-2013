...
#ifdef EZ2
ConVar sk_weapon_ar2_alt_fire_proto_radius( "sk_weapon_ar2_alt_fire_proto_radius", "5" ); //Breadman. Radius was 10.
ConVar sk_weapon_ar2_alt_fire_proto_duration( "sk_weapon_ar2_alt_fire_proto_duration", "1" ); // Breadman was 2.
#endif
...
// BUT NOW It's working as it should wat tha fack. DON'T QUESTION IT. SMILE. KEEP WORKING. 
//-----------------------------------------------------------------------------
void CWeaponAR2Proto::DelayedAttack( void )
{
	// Debug messages because DEBUG OKAY? JEEZE.
	//Msg("\n BURST NUMBER IS: %i", BurstMax);
	// Clamp
	...
	CreateCombineBall(	vecSrc, 
						vecVelocity, 
						sk_weapon_ar2_alt_fire_proto_radius.GetFloat(), 
						sk_weapon_ar2_alt_fire_mass.GetFloat(),
						sk_weapon_ar2_alt_fire_proto_duration.GetFloat(),
						pOwner );
...
void CWeaponAR2Proto::FireNPCSecondaryAttack( CBaseCombatCharacter *pOperator, bool bUseWeaponAngles )
{
	...
	float flAmmoRatio = 1.0f;
	float flDuration = RemapValClamped( flAmmoRatio, 0.0f, 1.0f, 0.5f, sk_weapon_ar2_alt_fire_proto_duration.GetFloat() );
